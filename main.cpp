...
vector<int> overlayAlgorithm(Image bottom_Layer, Image top_Layer) {
    ...
    int area = bottom_Layer.width * bottom_Layer.height;
    for (int pix=0; pix<area*3; pix++){
        float bottomLayer = (int)bottom_Layer.BGRvalues.at(pix);
        float topLayer = (int)top_Layer.BGRvalues.at(pix);
        float newPixel;

        if (bottomLayer/255.0f <= 0.5f){
            newPixel = 2 * bottomLayer/255.0f * topLayer/255.0f;
        }
        else{
            newPixel = 2 * (1-bottomLayer/255.0f) * (1-topLayer/255.0f);
            newPixel = 1 - newPixel;
        }
        newPixel *= 255.0f;
        newPixel += 0.5f;
        newPixelValues.push_back((int)newPixel);
    }
    return newPixelValues;
}
...
int main(int argc, char* argv[]) {
    if (argc < 2 || (strcmp(argv[1], "--help") == 0)) {
        ...
        cout << "Usage:" << endl<< "\t./project2.out [output] [firstImage] [method] [...]" << endl;
        return 0;
    }
...
// begin performing first operation
    ...
    while (index < argc) {
        ...
        if (newMethod == "screen" || newMethod == "subtract" || newMethod == "multiply" || newMethod == "overlay") {
            if(argc < index + 2){cout << "Missing argument." << endl; return 0;}
            string argAfterMethod = argv[index + 1];
        ...
        if (newMethod == "flip") {
            ...
            cout << "only green" << endl;
            newPixelValues = onlyGreen(newImage);
            index++;
        ...
        for (int j = 0; j < area * 3; j++) {
            newImage.BGRvalues.at(j) = newPixelValues.at(j); }
    }
